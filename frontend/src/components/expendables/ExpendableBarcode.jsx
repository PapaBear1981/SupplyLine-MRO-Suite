...
const ExpendableBarcode = ({ show, onHide, expendable }) => {
  ...
  useEffect(() => {
    const fetchBarcodeData = async () => {
      if (show && expendable) {
        ...
        setError(null);
        try {
          const response = await api.get(`/expendables/${expendable.id}/barcode`);
  ...
  const handlePrint = (type, containerRef) => {
    if (!containerRef.current || !barcodeData || !expendable) return;
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Expendable ${typeCapitalized} - ${expendable.part_number}</title>
          <style>
            ${printCSS}
          </style>
        </head>
  ...
  const completeExpendableData = barcodeData ? {
    ...expendable,
    ...barcodeData,
    created_at: barcodeData.created_at || expendable.created_at
  } : expendable;

  const barcodeString = barcodeData ? barcodeData.barcode_data : generateExpendableBarcodeData(expendable);
  const fields = barcodeData ? getExpendableBarcodeFields(completeExpendableData) : [];
  const title = barcodeData
    ? `${expendable.part_number} - ${barcodeData.lot_number ? `LOT: ${barcodeData.lot_number}` : `S/N: ${expendable.serial_number}`}`
    : `${expendable.part_number}`;

  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      <Modal.Header closeButton>
        <Modal.Title>Expendable Identification</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {loading && (
        ...
        {!loading && !error && barcodeData && (
          <Tabs defaultActiveKey="barcode" id="expendable-code-tabs" className="mb-3">
