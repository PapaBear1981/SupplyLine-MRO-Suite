  ...
  useEffect(() => {
    const fetchOverdueChemicals = async () => {
      try {
        ...
        setLoading(true);
        setError(null);
        const onOrderChemicals = await ChemicalService.getChemicalsOnOrder();
        console.log('OverdueChemicals: Received data:', onOrderChemicals);

        // Filter to only include chemicals with expected delivery dates in the past
        const overdue = onOrderChemicals.filter(chemical => {
