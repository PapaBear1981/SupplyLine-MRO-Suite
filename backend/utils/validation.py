...
def sanitize_string(value, max_length=None, allow_html=False):
    ...
    value = re.sub(r'[<>\"\'\\]', '', value) if not allow_html else value
    ...
    for field in date_fields:
        if field in data and data[field]:
            try:
                ...
                raise ValidationError(f"{field} must be a valid ISO format date")
...
def validate_schema(data, schema_name):
    ...
    # Import validation function from error_handler
