"""Migration script to add order management page permission."""
...
def run_migration():
    ...
    with app.app_context():
        print("Starting order management permission migration...")

        permission = Permission.query.filter_by(name="page.orders").first()
        if permission:
            print("  ⚠️  Permission 'page.orders' already exists")
        else:
            permission = Permission(
                name="page.orders",
                description="Access Order Management page",
                category="Page Access",
            )
            db.session.add(permission)
            db.session.commit()
            print("  ✅ Created permission: page.orders")

        target_roles = ["Administrator", "Materials Manager"]
        assignments = 0
        for role_name in target_roles:
            ...
            if existing:
                print(f"  ℹ️  Role '{role_name}' already has page.orders")
                continue

            db.session.add(RolePermission(role_id=role.id, permission_id=permission.id))
            assignments += 1
            print(f"  ✅ Granted page.orders to role '{role_name}'")
