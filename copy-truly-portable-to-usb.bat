@echo off
setlocal enabledelayedexpansion

echo ========================================
echo SupplyLine MRO Suite - Truly Portable USB Deployment
echo ========================================
echo.

REM Check if truly portable version exists
if not exist "SupplyLine-MRO-Suite-Truly-Portable" (
    echo ERROR: Truly portable version not found!
    echo Please run build-truly-portable.bat first.
    echo.
    pause
    exit /b 1
)

echo Available drives:
wmic logicaldisk get size,freespace,caption 2>nul | find ":"

echo.
set /p DRIVE_LETTER="Enter USB drive letter (e.g., E): "

REM Validate drive letter
if "%DRIVE_LETTER%"=="" (
    echo ERROR: No drive letter specified!
    pause
    exit /b 1
)

REM Remove colon if present and add it back
set DRIVE_LETTER=%DRIVE_LETTER::=%
set USB_DRIVE=%DRIVE_LETTER%:

REM Check if drive exists
if not exist "%USB_DRIVE%\" (
    echo ERROR: Drive %USB_DRIVE% does not exist!
    pause
    exit /b 1
)

REM Check drive type (optional warning)
for /f "tokens=2" %%i in ('wmic logicaldisk where "Caption='%USB_DRIVE%'" get DriveType /value 2^>nul ^| find "="') do set DRIVE_TYPE=%%i
echo Drive type: %DRIVE_TYPE% (2=Removable, 3=Fixed)
if not "%DRIVE_TYPE%"=="2" (
    echo WARNING: %USB_DRIVE% does not appear to be a removable drive
    set /p CONTINUE="Continue anyway? (y/N): "
    if /i not "!CONTINUE!"=="y" (
        echo Operation cancelled.
        pause
        exit /b 1
    )
)

echo.
echo Copying SupplyLine MRO Suite (Truly Portable) to %USB_DRIVE%...
echo This may take several minutes due to the embedded Python runtime...
echo.

REM Remove existing installation
echo Removing existing installation...
if exist "%USB_DRIVE%\SupplyLine-MRO-Suite" (
    rmdir /s /q "%USB_DRIVE%\SupplyLine-MRO-Suite" 2>nul
    if exist "%USB_DRIVE%\SupplyLine-MRO-Suite" (
        echo WARNING: Could not completely remove existing installation.
        echo Some files may be in use. Please close any running instances.
        pause
    )
)

REM Remove old launcher files
del "%USB_DRIVE%\Launch SupplyLine MRO Suite.bat" 2>nul
del "%USB_DRIVE%\README - SupplyLine MRO Suite.txt" 2>nul
del "%USB_DRIVE%\autorun.inf" 2>nul

echo Copying application files...
robocopy "SupplyLine-MRO-Suite-Truly-Portable" "%USB_DRIVE%\SupplyLine-MRO-Suite" /E /MT:4 /R:3 /W:1

REM Create USB-specific launcher
echo Creating USB launcher...
(
echo @echo off
echo setlocal
echo.
echo echo ========================================
echo echo SupplyLine MRO Suite - Truly Portable
echo echo ========================================
echo echo.
echo echo Starting from USB drive...
echo echo This may take a moment on first run.
echo echo.
echo.
echo REM Change to the application directory
echo cd /d "%%~dp0SupplyLine-MRO-Suite"
echo.
echo REM Run the truly portable launcher
echo call "Launch-SupplyLine-Truly-Portable.bat"
) > "%USB_DRIVE%\Launch SupplyLine MRO Suite - Truly Portable.bat"

REM Create autorun.inf for auto-launch support
echo Creating autorun.inf...
(
echo [autorun]
echo open=Launch SupplyLine MRO Suite - Truly Portable.bat
echo icon=SupplyLine-MRO-Suite\App\SupplyLine MRO Suite.exe
echo label=SupplyLine MRO Suite
echo action=Launch SupplyLine MRO Suite
) > "%USB_DRIVE%\autorun.inf"

REM Create comprehensive info file
echo Creating info file...
(
echo ========================================
echo SupplyLine MRO Suite - Truly Portable
echo ========================================
echo.
echo Version: 3.5.4
echo Build Date: June 2025
echo.
echo WHAT MAKES THIS "TRULY PORTABLE":
echo This version includes EVERYTHING needed:
echo - Python 3.11 runtime (embedded)
echo - All Python dependencies
echo - Complete Electron application
echo - SQLite database
echo - NO installation required on ANY Windows PC!
echo.
echo QUICK START:
echo 1. Double-click "Launch SupplyLine MRO Suite - Truly Portable.bat"
echo 2. Wait for the application to start (may take 30-60 seconds)
echo 3. Login with: ADMIN001 / admin123
echo.
echo ALTERNATIVE START:
echo 1. Navigate to SupplyLine-MRO-Suite folder
echo 2. Double-click "Launch-SupplyLine-Truly-Portable.bat"
echo.
echo SILENT START (no console window):
echo 1. Navigate to SupplyLine-MRO-Suite folder
echo 2. Double-click "Launch-SupplyLine-Silent.bat"
echo.
echo SYSTEM REQUIREMENTS:
echo - Windows 10 or later (64-bit)
echo - 4GB RAM minimum
echo - 500MB free disk space
echo - NO other software required!
echo - NO Python installation needed!
echo - NO Node.js installation needed!
echo.
echo FEATURES:
echo - Complete MRO management system
echo - Tool checkout/return tracking
echo - Chemical inventory management
echo - User management and RBAC
echo - Reporting and analytics
echo - Offline operation
echo - Embedded Python runtime
echo - Self-contained database
echo.
echo TECHNICAL DETAILS:
echo - Total size: ~300MB
echo - Python: 3.11.9 Embedded
echo - Backend: Flask with SQLite
echo - Frontend: React with Bootstrap
echo - Desktop: Electron 32.3.3
echo - Database: SQLite (portable)
echo.
echo TROUBLESHOOTING:
echo - First startup may be slow (Python initialization)
echo - If blocked by antivirus, add folder to exclusions
echo - Run as Administrator if permission issues occur
echo - Ensure port 5000 is available (backend server)
echo.
echo No installation required on target computers!
echo Just plug in USB and run!
echo.
) > "%USB_DRIVE%\README - SupplyLine MRO Suite - Truly Portable.txt"

echo.
echo ========================================
echo Truly Portable USB Deployment Complete
echo ========================================
echo.
echo The SupplyLine MRO Suite (Truly Portable) has been copied to %USB_DRIVE%
echo Total size: ~300MB (includes Python runtime)
echo.
echo Files created on USB:
echo   - SupplyLine-MRO-Suite\                              (Main application folder)
echo   - Launch SupplyLine MRO Suite - Truly Portable.bat   (USB launcher)
echo   - README - SupplyLine MRO Suite - Truly Portable.txt (Instructions)
echo   - autorun.inf                                         (Auto-launch support)
echo.
echo To use on ANY Windows computer:
echo 1. Insert the USB drive
echo 2. Double-click "Launch SupplyLine MRO Suite - Truly Portable.bat"
echo 3. Wait for startup (may take 30-60 seconds first time)
echo 4. Login with ADMIN001 / admin123
echo.
echo This version requires ZERO installation on target computers!
echo - No Python needed
echo - No Node.js needed  
echo - No dependencies needed
echo - Just plug and play!
echo.
echo Default login credentials:
echo   Username: ADMIN001
echo   Password: admin123
echo.
pause
